...
class PaymentIERequest extends TrxRequest {
	...
	public $request = array (
		...
		"IsBreakAccount" => "",
		"SplitAccTemplate" => "",
		/* 增加AccountNo，后台接口中，是request的属性，java中设置成了order的属性，这里与后台保持一致
       add by chj@20190312 */
		"AccountNo" => ""
	);
	...
	private function isValid() {		
	  ...
	  //add by chj@20190312 参照java修改，增加了支付账户长度的判断
		if (strlen($this->order["AccountNo"])!== 0 && strlen($this->order["AccountNo"]) < 10) 
			return "支付账户长度不能少于10位";
		//add by chj@20190312 参照java修改，增加了分账信息的判断
