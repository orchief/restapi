...
class QuickPaymentRequest extends TrxRequest {
	...
	private function isValid() {
		...
		//	return "支付账号不合法！";
		if (!DataVerifier::IsNumAndEnCh($this->request["CardNo"]))
      return "支付账户或账户别名不合法，请输入长度为6-30位字母数字的组合";
      
		if (!DataVerifier :: isValid($this->request["MobileNo"]))
			return "手机号不合法！";
		/*和java保持一致，增加手机号长度判断，需要为4位 add by chj@20190306*/
		if (strlen($this->request["MobileNo"])!== 4)
		  return "手机号长度不合法！";
		...
		/*和java保持一致，增加支付类型和支付渠道是否为空判断 add by chj@20190306*/
		if (empty($this->request["PaymentType"]))
        throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "未设定支付类型！");
