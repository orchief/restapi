...
class_exists('ICardType') or require (dirname(__FILE__) . '/core/ICardType.php');
...
class QuickAgentSignContractRequest extends TrxRequest {
	...
	protected function checkRequest() {
		...
		/*新增卡类型判断，和java保持一直 add by chj@20190306*/	
		if (!($this->request["CardType"] === ICardType :: PAY_TYPE_ABC) && !($this->request["CardType"] === ICardType :: PAY_TYPE_CREDIT) && !($this->request["CardType"] === ICardType :: PAY_TYPE_ALL))
		...
		if ($this->request["CardType"] === ICardType :: PAY_TYPE_CREDIT) {
			//验证贷记卡CVV2
			if (!DataVerifier :: isValidString($this->request["CVV2"], 3))
				throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "贷记卡CVV2长度不合法");
		  //验证贷记卡有效期
		  ...
		  //增加贷记卡CVV2和有效期的数字判断		
			if (!DataVerifier :: isValid($this->request["CVV2"]))
				throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "贷记卡CVV2不合法");
			if (!DataVerifier :: isValid($this->request["CardDueDate"]))
