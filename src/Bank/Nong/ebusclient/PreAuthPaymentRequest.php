...
class PreAuthPaymentRequest extends TrxRequest {
	...
	protected function checkRequest() {
		...
		/*如果手续费不为空，增加手续费的判断，与java保持一致 add by chj@20190307*/
		if(!empty($this ->order["Fee"]))
		{
		  if (!DataVerifier :: isValidAmount($this->order["Fee"], 2))
			  throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1101, "手续费金额不合法");	
	  }
	  ...
	  /*增加附言长度的判断，与java保持一致 add by chj@20190307 */
	  if(strlen($this ->order["MerchantRemarks"])>100)
	    throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1101, "附言长度超长");
	}
}
?>