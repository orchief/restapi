...
class AgentPaymentRequest extends TrxRequest {
	...
	/*增加分账信息，与java保持一致  add by chj@20190307*/
	...
	protected function checkRequest() {
		...
		{
			unset ($this->request["InstallmentCode"]);
			unset ($this->request["InstallmentNum"]);
		}
		
		//增加支付渠道判断，与java保持一致  add by chj@20190307
    if (empty($this->request["PaymentLinkType"]))
      throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "未设定支付渠道！"); 
    ...
    //add by chj@20190307 参照java修改，增加了分账信息的判断
    if(($this->request["IsBreakAccount"] === IIsBreakAccountType :: IsBreak_TYPE_NO)&& count($this->splitaccinfos)>0)
    {
    	return "分账标志为0时，不能设置分账信息";
    }
    //增加了证件号的判断--与后台确认，这个字段为必输的 add by chj@20190307
    if (empty($this->request["CertificateNo"]))
      throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "未设定证件号！"); 
    if (strlen($this->request["CertificateNo"])!==15 && strlen($this->request["CertificateNo"])!==18)
      throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "证件号不合法！"); 
